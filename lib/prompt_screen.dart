...
//Funtion to submit mood and genres and fetch playlist
  Future<void> _submitSelection() async {
    ...
    // Initialize the Gemini model and make API request
    ...
    try {
      ...
      // Assuming response has a 'text' property to directly access the result
      final playlistString = response.text ?? '';

      setState(() {
        final songPattern = RegExp(r'^\d+\.\s\*\*(.+)\*\*$');
        _playlist = playlistString.split('\n').where((song) {
          // Only include lines that match the song pattern
          return songPattern.hasMatch(song);
        }).map((song) {
          // Extract song details from the matched line
          final match = songPattern.firstMatch(song);
          if (match != null) {
            final songDetails = match.group(1) ?? '';
            final parts = songDetails.split(':');
            if (parts.length >= 2) {
              return {
                'artist': parts[0].trim(),
                'title': parts[1].trim(),
